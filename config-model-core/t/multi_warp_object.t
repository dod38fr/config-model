...
is( $root-> is_element_available(name => 'bar', role => 'advanced'), 0,
...
is( $root-> is_element_available(name => 'bar', role => 'advanced'), 1,
  'check element bar for advanced user (now available)') ;

$root->load(step => 'bar:1 X=Av', role => 'master') ;
...
my @array = $root->get_element_for('bar')-> get_all_warper_object ;
