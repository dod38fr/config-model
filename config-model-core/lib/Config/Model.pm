...
use vars qw(@status @level @experience_list %experience_index %default_property) ;
...
sub check_class_parameters {
    ...
    foreach my $info_name (@legal_params) {
        ...
        while (@info) {
            ...
            if ($info_name =~ /description|level|summary|experience|status/) {
                map {$model->{element}{$_}{$info_name} ||= $info 
                    || $default_property{$info_name};}
                         @element_names ;
            }

            if ($info_name eq 'element' and defined $info->{permission}) {
