...
=head1 Popcon configuration file

A quick looks in C</etc> directory shows that Popcon's configuration
...
all the required information to create Popcon's configuration model.

=head1 Popcon documentation

Let's start from C<default.conf> file. Since this file is loaded
by popconf I<before> loading C</etc/popularity-contest.conf</t>,
...
define the structure of the file. Here, the PopCon class is followed by
...
In the description above, the chimping marker '-' after '>' is used to
...
=item * right-click on the created item (I<0>)
...
I<ShellVar> since I<popularity-contest.conf> is made of shell
variables.
...
will be shown the Popcon configuration editor GUI. The same that your
users will get.

If everything is fine, you can quit the model editor (menu File->quit)

=head1 Specify the installation
