...
References. For instance, in C<Xorg::Device::Radeon>, I<Monitor-DVI-0>
name must refer to one of the monitors declared in I<Monitor> section.
...
changing a driver name from I<fglrx> to I<radeon>, some options will
...
I<LocalForward> parameter.

But this bind address is ignored if I<GatewayPorts> is false (which is
the default).

In order to present only meaningful parameters to the user,
I<bind_address> parameter must be hidden when I<GatewayPorts> is false
and shown when I<GatewayPorts> is true.

Here's the recipe. First creeate a boolean element for C<GatewayPorts>:
...
And I<LocalForward> that will provide I<bind_address> parameter:
...
In I<Ssh::PortForward> configuration class, declate I<bind_address> with the warp
instructions:
