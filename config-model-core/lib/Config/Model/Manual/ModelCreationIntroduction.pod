...
model. Creation of more complex models are descfribed in 
...
Square brackets C<[ ... ]> contain a list of items (I<array> or
I<list> in Perl land)
...
identified by their I<Identifiers>, the configuration can be
...
(i.e.I<Device> and I<Monitor> value in I<Screen> section) and so they
...
instance, I<Device> options in C<Xorg.conf> are different depending on
the value of the I<Device Driver>. In this case, the structure of the
...
class is required (let's say the I<Approx> class). This class will
...
I<Xorg::Screen::Option> for the Screen options

=item * 

I<Xorg::Screen::Display> for theI<Display> subsection
...
TheI<Xorg::Screen> class will be:
...
instance, for model I<Xorg>, you must create
C<./lib/Config/Model/models/Xorg.pl>. Other classes
likeI<Xorg::Screen> can be stored in their own file
...
for I<Config::Model::Itself> model editor.
...
(with I<level> and I<status> and I<experience>) and documentation
(I<summary> andI<description>).
...
I<description>: full length description of the attribute

=item *

I<summary>: one line summary of the above description

=item *

I<level>: is I<important>, I<normal> or I<hidden>. The level is used
...
Set to I<leaf> (mandatory)

=item value_type

Either I<boolean>, I<integer>, I<number>, I<enum>, I<string>,
I<uniline> (i.e. a string without "\n") (mandatory)

=item min

Mininum value (for I<integer> or I<number>)

=item <max

Maximum value (for I<integer> or I<number>)
...
For instance, I<AddressFamily> parameter (sshd_config(5)) is specified
...
For Config::Model, I<AddressFamily> is a type I<leaf> element,
value_type I<enum> and the application will use I<any> if this
...
So let's use I<distribution> as a parameter name.
...
The model will need to declare that I<distrib> is:

=over

=item *

a type I<hash> parameter
...
the values of the hash are of type I<leaf> and value_type I<uniline>
...
In the Xorg example above, the options of I<Xorg::Screen> need their
own subranch in the tree:
...
So the I<Option> node element is declared with:
...
And the Xorg model must define these 2 parameters as I<hash>. The
cargo of this hash will of type I<node> and will refer to 2 different
configuration classes, one for I<Device> (C<Xorg::Device>) and one for
I<Screen> (C<Xorg::Screen>):
...
parameter). The exploration will also respect the I<experience>
parameter. I.e. a wizard run with I<master> experience (see
Option->Experience menu in the Perl/Tk interface) will show more
parameters than running the interface with I<beginner> experience.
...
I<experience> to the right level, either master, advanced or beginner
(default).

=item *

The importance level of the parameter (important, normal or
hidden). I<level> is used to set how configuration data is presented
...
Config::Model own serialisation format (a bit like YAML). 
...
Even the read method can haave an arbitrary name by specifying a
...
Usage of different librairies (e.g. L<Augeas|http://augeas.net> or pure Perl backend)

=back

For instance, to try Augeas and fall back on a custom class in acse of problem, specify:
...
of the times, the I<read> and I<write> specification will be
identical. In this case, there's no need to enter them: the data
specified in the I<read> specification will be used to write the
configuration file.
...
Several I<write> specification can be used. They are tried in order,
...
By combining multiple read specification with I<'one>' write
...
Config::Model tries to read the config with I<perl_file> read backend
...
I<write_config> backend which writes C</etc/my_cfg/example.pl>

=item 1.

At the next invocation, the first I<read> backend will successfully
read the perl configuration file. The old file is left alone and can
be removed later by the sys admin.
