[
          {
           ...
           kernel log for "AgpCommand = hex-integer". The second last hex
           digit should be 0 (zero) if fast-writes is off, or 1 (one) if it is
           on.'
                           },
                           'AGPv3Mask',
                           {
...
"0x00000002" Disable AGP 8x (forces 4x).'
           ...
           program that makes use of OpenGL and further when the fault can be
           traced down to the OpenGL implementation of the graphics driver,
           then you should try to tune the TLS settings.'
                           },
                           'UseInternalAGPGART',
                           {
                             'value_type' => 'enum',
